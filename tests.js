var entities = [
  {
    ...
    create: (function() {
      ...
      return function() {
        counter++;
        return createUser(counter + [], counter + []);
      };
    })(),
    createFn: createUser,
    remove: deleteUser,
    ...
    inGroup: function(user, group) {
      return userGroups(user).indexOf(group) != -1;
    }
  },
  {
    name: "right",
    ...
    inGroup: function(right, group) {
      return groupRights(group).indexOf(right) != -1;
    }
  },
  {
    name: "group",
...
entities.forEach(function(entity) {
  ...
  describe("Функция " + entity.remove.name, function() {
    ...
    it("должна бросить исключение, если ей передали плохой аргумент", function() {
      ...
      expect(function() {
        entity.remove(undefined);
      }).toThrowError(Error);
    });

    it(
      "должна бросить исключение, если ей передали уже удаленн(ого/ое/ую) " +
        entity.name,
      function() {
        var entityToRemove = entity.create();
        entity.remove(entityToRemove);
  ...
  if (entity.addToGroup) {
    describe("Функция " + entity.addToGroup.name, function() {
      ...
      it(
        "должна добавлять несколько " +
          entity.name +
          " в группу последовательно",
        function() {
          var createdEntities = [
            entity.create(),
            entity.create(),
          ...
          var group = createGroup();
          createdEntities.forEach(function(createdEntity) {
            entity.addToGroup(createdEntity, group);
          });
          createdEntities.forEach(function(createdEntity) {
            expect(entity.inGroup(createdEntity, group)).toBe(true);
      ...
      it("должна бросить исключение, если ей передали плохие аргументы", function() {
        ...
        params.forEach(function(args) {
          expect(function() {
            entity.addToGroup.apply(null, args);
          }).toThrowError(Error);
        });
      });
      ...
      it("должна бросить исключение, если ей передали что-то удаленное", function() {
        ...
        deleteGroup(group);
        expect(function() {
          addRightToGroup(secondEntity, group);
        }).toThrowError(Error);
      });
    });
  }
  ...
  if (entity.removeFromGroup) {
    describe("Функция " + entity.removeFromGroup.name, function() {
      ...
      it(
        "должна должна бросить исключение при попытке удалить " +
          entity.name +
          " из группы, которого там нет",
        function() {
          var createdEntity = entity.create();
          var group = createGroup();
      ...
      it(
        "должна удалить " + entity.name + " только из группы (а не вообще)",
        function() {
          var createdEntity = entity.create();
          var group = createGroup();
          entity.addToGroup(createdEntity, group);
      ...
      it("должна бросить исключение, если ей передали плохой аргумент", function() {
        ...
        params.forEach(function(args) {
          expect(function() {
            entity.removeFromGroup.apply(null, args);
          }).toThrowError(Error);
        });
      });
      ...
      it("должна бросить исключение, если ей передали что-то удаленное", function() {
        ...
        entity.remove(firstEntity);
        expect(function() {
          entity.removeFromGroup(firstEntity, group);
        }).toThrowError(Error);

        deleteGroup(group);
        expect(function() {
          entity.removeFromGroup(secondEntity, group);
        }).toThrowError(Error);
      });
    });
  }
...
function consistentState() {
  ...
  //   return (
  //     result &&
  //     userGroups(user).reduce(function(result, group) {
  //       if (groups().indexOf(group) == -1) throw [user, group];
  ...
  //   return (
  //     result &&
  //     groupRights(group).reduce(function(result, right) {
  //       if (rights().indexOf(right) == -1) throw [right, group];
...
var actions = [
  ...
  function() {
    var usersWithGroups = users().filter(function(user) {
      return userGroups(user).length > 0;
    });
  ...
  function() {
    var groupsWithRights = groups().filter(function(group) {
      return groupRights(group).length > 0;
    });
    if (groupsWithRights.length > 0) {
      var group = randomElement(groupsWithRights);
      var right = randomElement(groupRights(group));
      removeRightFromGroup(right, group);
    }
  }
];
